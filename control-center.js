...
function prepareMapLayout(){
  ...
  if(version)version.textContent='CONTROL CENTER 14.4.1';
...
function generateReport(){
  ...
  if(!preview){
    console.error('Contenitore #reportPreview non trovato.');
    return;
  }
  if(!live){
    preview.innerHTML='<p class="empty-state error">Dati non ancora disponibili. Premi Aggiorna e riprova.</p>';
    return;
  }
  const c=summary('Comune'),m=summary('Municipio'),top=rankingRows('Comune').slice(0,10);
...
$('#loginForm').addEventListener('submit',async e=>{
  ...
  if(generateReportBtn){
    generateReportBtn.addEventListener('click',()=>{
      ...
      const originalLabel=generateReportBtn.textContent;
      generateReportBtn.textContent='Generazione…';
      try{
        generateReport();
      }catch(error){
        ...
        generateReportBtn.textContent=originalLabel;
      }
    });
  }else{
    console.error('Pulsante #generateReportBtn non trovato.');
  }
