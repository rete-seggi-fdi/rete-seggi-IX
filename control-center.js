...
let mapInstance=null,mapMarkers=[],leafletPromise=null;
...
function ensureMapContainer(){let el=$('#mapCanvas');if(el)return el;const list=$('#mapSectionList');if(!list)return null;el=document.createElement('div');el.id='mapCanvas';el.setAttribute('aria-label','Mappa dei plessi elettorali');el.style.cssText='height:560px;min-height:420px;width:100%;border-radius:18px;overflow:hidden;margin:0 0 18px;background:#e9ecef;box-shadow:0 8px 28px rgba(0,0,0,.12)';list.parentNode.insertBefore(el,list);const info=document.createElement('p');info.id='mapGeoSummary';info.style.cssText='margin:0 0 12px;font-size:.92rem;opacity:.8';el.parentNode.insertBefore(info,el);return el}
